<template>
  ...
  <div class="flex flex-col container min-h-screen p-6 bg-white shadow">
    ...
    <div class="text-center flex flex-col items-center">
        ...
        <div class="mb-4">
            <img class="w-40 h-40 rounded-full border-4 border-blue-500" :src="craftsman.image_url" alt="Craftsman Profile" />
        </div>
        <div class="mb-4">
            <h2 class="text-gray-800 font-bold text-4xl">{{ craftsman.firstName }}</h2>
        </div>
        <!-- عرض تفاصيل الحرفة والمكان والسعر -->
        <div class="mb-4 space-y-2" dir="rtl">
            <p class="text-gray-700">{{ craftsman.crafts.type }} :الحرفة</p>
        ...
        <div class="mb-8 flex flex-col items-center justify-center cursor-pointer" @click="showModal = true">
            <star-rating :rating="craftsman.rating" :read-only="true" starSize="3"></star-rating>
            <p class="text-sm text-gray-600 mt-2">عدد العملاء الذين تعاملوا مع الحرفي: {{ craftsman.number_clients }}</p>
        </div>
      </div>
    ...
    async showInformation(){
      try {
        const response =await axios.post(`/show_information_craftsman`);
			  this.craftsman=response.data.data;
			} catch (error) {
				console.log(error);
			}     
    },
