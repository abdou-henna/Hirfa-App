      ...
      <div>
        ...
        <section class="w-[550px] mx-10 mt-10 h-[75%] grid content-center">
          ...
          <div dir="rtl" class="mx-6">
            ...
            <button @click="submitLogin()" type="submit" class="hover:bg-[#004aca] w-full mt-12 mb-4 bg-[#005CFF] font-semibold text-white h-[50px] rounded-md focus:bg-[]">تسجيل الدخول</button>
...
methods: {
  async submitLogin() {
    ...
    /*try {
      const response = await axios.post('YOUR_LOGIN_API_ENDPOINT', this.loginData);
      // هنا يمكنك التعامل مع الاستجابة، مثل تخزين token أو إعادة التوجيه إلى صفحة أخرى
